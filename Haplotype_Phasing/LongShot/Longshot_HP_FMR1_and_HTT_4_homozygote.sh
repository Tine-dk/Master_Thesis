...
# Find VCF filer fra LongShot phasningen
	for file in longshot_ny/merged_vcf/*.vcf.gz; do
		...
		bcftools view -g hom longshot_ny2/longshot_vcf_subset_filtered/${SAMPLE}_${gene}_longshot_Filtered_SNVs_window${Window}_DP${DP}.vcf > longshot_ny2/longshot_vcf_subset_filtered_hom/${SAMPLE}_${gene}_longshot_Filtered_hom_SNVs_window${Window}_DP${DP}.vcf

		#Extract Illumina Genotypes from the filtered vcf file
		grep -v '^##' longshot_ny2/longshot_vcf_subset_filtered_hom/${SAMPLE}_${gene}_longshot_Filtered_hom_SNVs_window${Window}_DP${DP}.vcf | cut -f 1,2,4,5 | sed 's/#//g' > \
		longshot_ny2/longshot_vcf_hom_genotypes/${SAMPLE}_${gene}_longshot_hom_genotypes_window${Window}_DP${DP}.txt
		...
		snvs=$(bcftools view -g hom longshot_ny2/longshot_vcf_subset_filtered/${SAMPLE}_${gene}_longshot_Filtered_SNVs_window${Window}_DP${DP}.vcf | grep -v '^#' | wc -l)
		echo $snvs
		...
		if [ $snvs -gt 0 ]; then
			python /work/sdularsen/tine/nanoporeData/kommandoer/Longshot_SNV_Base_Identification4_homozygote.py \
