...
snv_positions = set()
with open (path + "Illumina_data/"+sample+"_"+gene+"_Illumina_Dragen_Filtered_SNVs_window"+window+"_AF_0_"+AF_low+"_to_0_"+AF_high+"_DP"+DP+".vcf", "r") as f:
    for line in f:
        if not line.startswith("#"):
            snv_positions.add(int(line.split("\t")[1]))

# Load read names into a list to be used in the data frame
# Read names bør kun indlæses for window = 0, ik? Ellers får vi da reads med der ligger udenfor repeat området?
read_names = list()
with open (path + "alignedMinimap2/"+sample+"_"+gene+"_Read_Name_window"+window+"_AF_0_"+AF_low+"_to_0_"+AF_high+"_DP"+DP+".txt", "r") as f:
    for line in f:
...
bamfile = pysam.AlignmentFile(path + "alignedMinimap2/"+sample+"_"+gene+"_window"+window+"_AF_0_"+AF_low+"_to_0_"+AF_high+"_DP"+DP+".bam", "rb")
...
snv_df.to_csv(path + "Illumina_data/"+sample+"_"+gene+"_bases_window"+window+"_AF_0_"+AF_low+"_to_0_"+AF_high+"_DP"+DP+".txt", sep=',', mode='w',na_rep='NA')
